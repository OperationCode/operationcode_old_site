...
describe Slack::Client do
  ...
  describe 'inviting a user' do
    before(:all) do
      @post_url = URI.join("https://#{@slack_client.domain}", Slack::INVITE_PATH).to_s
    end
    it 'returns true if all is well' do
      stub_request(:post, /#{Regexp.quote(@post_url)}.*/)
  ...
  describe 'posting a message' do
    before(:all) do
      @post_url = URI.join("https://#{@slack_client.domain}", Slack::POST_MESSAGE_PATH).to_s
    end

    it 'returns true if all is well' do
      stub_request(:post, /#{@post_url}.*/)
        .to_return(:status => 200, :body => '{"ok": true}', :headers => {})
      res = @slack_client.post_message_to(channel: 'test', with_text: 'hello world')
      expect(res).to equal true
    ...
    it 'raises a PostMessageFailed if OK is not true' do
      stub_request(:post, /#{Regexp.quote(@post_url)}.*/)
        .to_return(:status => 200, :body => '{"ok": "lolsorandom"}', :headers => {})

      expect{@slack_client.post_message_to(channel: 'test', with_text: 'hello world')}.to raise_error(Slack::Client::PostMessageFailed)
    end

    it 'raises a RequestFailed error on 500 HTTP codes' do
      stub_request(:post, /#{Regexp.quote(@post_url)}.*/)
        .to_return(:status => 500, :body => '{"ok": false}', :headers => {})
      expect{@slack_client.post_message_to(channel: 'test', with_text: 'hello world')}.to raise_error(Slack::Client::RequestFailed)
    end
  end
end
